...
- `src/db.js` — `/{COIN}/api/status` (`getStatus()`) now reports the chain-tip reference and indexer lag, not just the indexer's own position. The response previously carried only `last_block` and `last_block_time` (the highest block the indexer had processed and its block_time), so a caller could see where the indexer was but could not tell a stalled indexer from a healthy one without a separate out-of-band query against the node or decoder — a stalled indexer reported the same `last_block` indefinitely with no visible lag signal. Added two per-coin maps: `node_tip[coin]`, the chain tip sourced from the decoder's highest seen block (the decoder records every block up to the node tip), and `lag_blocks[coin]`, computed as `node_tip - last_block` (clamped to >= 0, since the indexer reads from the decoder and can never lead it). The tip is read by reusing the existing indexer connection pool with a database-qualified query — no new connection or dependency — and only when the decoder DB is on the same server/credentials as the indexer DB (recorded in a new `decoderDb` map during `setupConnectionPools()`); when the decoder tip is unknown or its query fails, `node_tip`/`lag_blocks` are `null` for that coin and `last_block`/`last_block_time` are unaffected. The change is purely additive; existing callers reading only `last_block`/`last_block_time` continue to work unchanged.
