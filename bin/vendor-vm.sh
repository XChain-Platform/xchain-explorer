...
#
# Refresh (or check) the vendored copy of xchain-vm used by the explorer.
#
# The explorer depends on xchain-vm via "file:./xchain-vm" (optionalDependencies
# in package.json): a copy staged inside this repo so the Docker build context is
# self-contained. The dependency is OPTIONAL because the explorer only needs the
# VM for the flag-gated contract simulation endpoint (EXPLORER_VM_QUERY_ENABLED);
# on hosts where the isolated-vm native build is unavailable the explorer still
# runs and the endpoint reports VM_MODULE_UNAVAILABLE. In a normal rollout,
# xchain-node stages the copy fresh on every `update` (ModuleService buildAndUp,
# LIBRARY_BUNDLES); this script covers local development from the monorepo.
#
...
# missing two consensus gates (STATE_KEY_NUL, METERING_EVAL_ORDER) and a wrong
# async-gate flag-day while still declaring the same CONSENSUS_VERSION.
