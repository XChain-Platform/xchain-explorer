...
// action detail render suite boots it. The page's own script never runs.
function bootPage(){
...
describe('markup labels still render as markup through formatLinkHtml', function(){
    ...
    it('keeps the search highlight on a token hit while escaping the tick', function(){
        const win = bootPage();
        win.XC.query = 'TAMP';
        const row = win.jQuery('<tr><td></td><td></td><td></td><td></td></tr>')[0];
        win.xcDatatableRenderSearchRow({ row: row, data: [1, TICK, 'desc'], coin: 'TDOGE', type: 'token' });
    ...
    it('renders a market pair with its icons and both ticks as text', function(){
        const win = bootPage();
        const row = win.jQuery('<tr><td></td><td></td><td></td><td></td><td></td><td></td><td></td></tr>')[0];
        win.xcDatatableRenderMarketRow({
            row: row, data: [1, TICK, 'TDOGE', '1', '1', '1', '1', '0'], coin: 'TDOGE'
        });
        const cell = win.jQuery('td', row).eq(1);
