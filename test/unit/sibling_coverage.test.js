...
// like it did.
//
// A dozen guards across test/unit and test/conformance are byte-identity or
// value-parity checks against a SIBLING checkout (abi-core and the schema
// against xchain-indexer, proof serving against xchain-sdk, the vendored coins
// registry against xchain-hub, and so on). Every one of them resolves its
...
// this repo's then-declared siblings ran 2431 passing / 18 pending where the
// full checkout runs 2444 / 5, so 13 guards had silently not run.
//
...
const SIBLINGS = [
    ...
    { repo: 'xchain-sdk', envs: ['XCHAIN_SDK_DIR'],
      marker: 'src',
      guards: 'checkpoint, locked-balance and contract-state proof parity' },
    { repo: 'xchain-decoder', envs: ['XCHAIN_DECODER_ROOT'],
      marker: path.join('src', 'sql'),
