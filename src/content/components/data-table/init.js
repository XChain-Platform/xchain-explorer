...
var xcDataTableInitColumns = null;

function xcDataTableInitRegister(){
    'use strict';
    ...
    xcDataTableInitColumns = (typeof XCDataTableColumns !== 'undefined')
        ? XCDataTableColumns
    ...
    reg.register('data-table', {
        ...
        mount: function(el, props, ctx){
            return xcDataTableInitMount(xcDataTableInitColumns, el, props, ctx);
        }
    });
}
...
function xcDataTableInitPermuteRow(tr, order){
    if(!tr) return;
    var cells = [];
    var i;
...
function xcDataTableInitPermuteTable(table, columns){
    if(!xcDataTableInitColumns || !xcDataTableInitColumns.needsPermutation(columns)) return;
    var order = xcDataTableInitColumns.renderOrder(columns);
    ...
    if(head && head.getAttribute('data-xc-permuted') !== '1'){
        xcDataTableInitPermuteRow(head, order);
    ...
    for(var i = 0; i < rows.length; i++){
        ...
        xcDataTableInitPermuteRow(rows[i], order);
        rows[i].setAttribute('data-xc-permuted', '1');
    }
}

function xcDataTableInitMount(cols, el, props, ctx){
    ...
    loadDatatablesData(coin, props.action, props.query || null, props.type || null, {
        ...
        onDraw: function(){ xcDataTableInitPermuteTable(table, props.columns); }
    });
