 ...
 **********************************************************************
 *
 * XChain Explorer: Equivocation Header (EQUIV_HEADER / WI-2 bump 2)
 *
 ...
 *
 * The explorer re-derives the checkpoint canonical (the /checkpoint/:idx/verify
 * endpoint) to verify checkpoint signatures it serves, so it MUST flip on the same
 * flag-day as the hub + indexer or it fails to verify post-activation signatures.
 * Adding `<VIEW>` makes equivocation provable WITHOUT false-positiving honest view
 * changes (which re-sign different content for the same round under a different view).
 * Equivocation = same (engine, round, view), different content.
 *
