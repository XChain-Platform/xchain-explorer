...
// Content-Security-Policy for the custom-content viewer document, replacing the
// app-wide policy on that one response (processContentViewerRequest). Token art is
// author-written HTML that commonly embeds a player or canvas from some other host,
// so every fetch directive admits https:, data: and blob: - what contains this
// content is the opaque origin the `sandbox` directive forces (no allow-same-origin
// anywhere), never the fetch directives. 'self' is deliberately absent: in an opaque
// origin it matches nothing, so listing it would only read as protection that isn't
// there. frame-ancestors is the exception, matched against this response's URL, and
// keeps another site from embedding the viewer to borrow this policy.
const CONTENT_VIEWER_CSP = [
    "sandbox allow-scripts allow-popups allow-forms allow-modals allow-presentation",
    "default-src https: data: blob:",
    "script-src 'unsafe-inline' 'unsafe-eval' 'wasm-unsafe-eval' https: data: blob:",
...
class XChainExplorer {
    ...
    setupUrls(){
        ...
        // Host document for a token's custom HTML, loaded into the sandboxed viewer
        // on token.html. It is a route rather than an html-table page because it
        // needs its own CSP: see processContentViewerRequest().
        this.app.get('/content-viewer', (req, res) => { this.processContentViewerRequest(req, res); });
    ...
    // Serve the host document for a token's custom HTML (src/content/sandbox/
    // content-viewer.html), with a policy of its own rather than the app-wide one
    // Helmet set on the way in.
    //
    // Why it cannot be a srcdoc string on token.html, which is what it replaced: a
    // srcdoc, blob: or data: document INHERITS its embedder's CSP, so token art
    // that embeds a third-party player or canvas was refused by this app's
    // frame-src ('self', youtube, soundcloud) and rendered as a broken-page
    // placeholder. A document fetched over the network inherits nothing and is
    // governed by the header below instead, which the embedder's policy still has
    // to admit - frame-src 'self' covers this same-origin route.
    //
    // The policy is loose about what the art may LOAD and strict about what it may
    // BE. `sandbox` without allow-same-origin forces an opaque origin even if the
    // route is opened directly in a tab, so nothing served here can read the
    // explorer's cookies, storage or DOM; the iframe carries the same sandbox as
    // an attribute, and the two intersect. frame-ancestors keeps other sites from
    // embedding the viewer to borrow this policy for their own content.
    processContentViewerRequest(req, res){
