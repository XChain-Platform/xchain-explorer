...
// the same assertions against xchain-explorer's own copy, express version and
...
describe('observability/metrics: exposition format', function () {
    ...
    it('collectDefaultMetrics exposes process and service identity', function () {
        const reg = new Registry();
        collectDefaultMetrics(reg, { service: 'xchain-explorer', version: '1.2.3', coin: 'BTC', network: 'regtest' });
        const out = reg.render();
        expect(out).to.include('xchain_service_info{service="xchain-explorer",version="1.2.3",coin="BTC",network="regtest"');
...
describe('observability/logShipper', function () {
    ...
    it('emits NDJSON with the envelope keys when LOG_FORMAT=json', function () {
        const sink = fakeConsole();
        const log = createLogShipper({ service: 'xchain-explorer', version: '9.9.9', env: { LOG_FORMAT: 'json' }, console: sink });
        log.warn('block stalled', { height: 42 });
        const rec = JSON.parse(sink.lines.warn[0]);
        expect(rec.level).to.equal('warn');
        expect(rec.service).to.equal('xchain-explorer');
...
describe('observability/installObservability', function () {
    ...
    it('registers NO route when the flag is unset, but still hands back a registry', async function () {
        ...
        const obs = installObservability(app, { service: 'xchain-explorer', env: {}, console: fakeConsole() });
    ...
    it('serves the exposition text and instruments requests when enabled', async function () {
        ...
        const obs = installObservability(app, {
            service: 'xchain-explorer', version: '1.0.0', coin: 'BTC', network: 'regtest',
        ...
        try {
            ...
            expect(body).to.include('xchain_service_info{service="xchain-explorer",version="1.0.0",coin="BTC",network="regtest"');
...
describe('observability/patchConsole', function () {
    ...
    it('routes console.* through the shim, mapping log to info', function () {
        const handle = patchConsole({ service: 'xchain-explorer', env: {} });
        const seen = [];
        ...
        console.error('bad');
        unpatchConsole();
        expect(seen[0]).to.match(/ info \[xchain-explorer\] plain$/);
        expect(seen[1]).to.match(/ warn \[xchain-explorer\] careful$/);
        expect(seen[2]).to.match(/ error \[xchain-explorer\] bad$/);
    });
    ...
    it('renders a multi-line message as ONE line with the breaks escaped', () => {
        ...
        expect(sink.lines.error[0]).to.match(/^\d{4}-\d{2}-\d{2}T[\d:.]+Z error \[xchain-explorer\] DB write failed:/);
    });
    ...
    it('JSON mode keeps the real newlines and still emits one physical line', () => {
        const sink = fakeConsole();
        const log = createLogShipper({ service: 'xchain-explorer', env: { LOG_FORMAT: 'json' }, console: sink });
        log.error('line one\nline two');
        expect(sink.lines.error).to.have.lengthOf(1);
        expect(sink.lines.error[0]).to.not.match(/\n/);
