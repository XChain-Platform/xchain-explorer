...
### Changed
- A coin whose indexed tip has fallen behind is served with a freshness marker (response headers, a `freshness` body field, `stale` on `/status`, `stale: true` on WebSocket frames) and a delayed-data banner on every page, instead of every data route answering 503; `EXPLORER_STALE_FAIL_CLOSED=1` restores the refusal.
